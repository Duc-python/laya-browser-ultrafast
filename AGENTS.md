...
- The decision backend (Laya local by default, TypeSafe hosted as legacy) chooses an operation and operation-specific target heads in one request. Consume only the selected operation's target.
...
Checks: uv run ruff check ., uv run pytest, node --check laya_browser_ultrafast/static/app.js, uv build.
